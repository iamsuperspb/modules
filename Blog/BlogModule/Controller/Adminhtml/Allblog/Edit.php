...
class Edit extends \Magento\Backend\App\Action
{
    ...
    protected function _initAction()
    {
        ...
        $resultPage->setActiveMenu('Blog_BlogModule::blog_allblog')
            ->addBreadcrumb(__('Blog'), __('Blog'))
            ->addBreadcrumb(__('Manage All Blog'), __('Manage All Blog'));
        return $resultPage;
    }
    ...
    public function execute()
    {
        ...
        $this->_coreRegistry->register('blog_allblog', $model);

        // 5. Build edit form
        /** @var \Magento\Backend\Model\View\Result\Allnews $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Blog') : __('Add Blog'),
            $id ? __('Edit Blog') : __('Add Blog')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Allblog'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getTitle() : __('Add Blogs'));
